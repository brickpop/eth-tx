...
function wrapContract(abi, byteCode) {
  ...
  abi.forEach(({ constant, name, inputs, type }) => {
    ...
    if (type === "function") {
      const constFuncCall = function() {
        ...
        return sendContractConstantTransaction(self.$contract, name, opts);
      };
      if (!constant) {
        WrappedContract.prototype[name] = function() {
          ...
          return sendContractTransaction(self.$contract, name, opts);
        };
      } else {
  ...
  WrappedContract.new = function() {
    ...
    return deployContract(opts).then(contract => {
      ...
      return new self(contract._address || contract.address);
    });
  };
