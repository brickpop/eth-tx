...
function sendTransaction({
  ...
  return new Promise((resolve, reject) => {
    ...
    // txOpts.$gas = 4700000;
    web3.eth.estimateGas(txOpts, (err, estimatedGas) => {
      if (err) return reject(err);
      else if (estimatedGas >= 4700000)
        return reject(
...
function sendContractTransaction(contract, method, opts = {}) {
  if (!contract) return Promise.reject(new Error("The contract is required"));
  if (!method) return Promise.reject(new Error("The method is required"));
  var fromAccount, gas;
  var errors = [];
  ...
  return Promise.try(() => {
    ...
    .then(
      txHash =>
        new Promise((resolve, reject) => {
          web3.eth.getTransactionReceipt(txHash, err => {
            if (err) return reject(err);
            resolve(txHash);
          });
        })
...
function getTransactionReceipt(web3, txHash) {
  return new Promise((resolve, reject) => {
    web3.eth.getTransactionReceipt(txHash, (err, data) => {
      if (err) return reject(err);
      else resolve(data);
    });
  });
...
function getBlock(web3, blockNumber) {
  return new Promise((resolve, reject) => {
    web3.eth.getBlock(blockNumber, (err, data) => {
      if (err) return reject(err);
      else resolve(data);
    });
  });
...
function getAccounts() {
  return new Promise((resolve, reject) => {
    web3.eth.getAccounts((err, accts) => {
      ...
      accounts = accts; // update the current list
      resolve(accts);
    });
  });
}
